...
import { Activity, BrainCircuit, Play, Route } from "lucide-react";
...
export function CinematicHero() {
  ...
  async function handleSearch(query: string, durationFilter: VideoDurationFilter) {
    ...
    trackAnalyticsEvent("search_submitted", { query, duration_filter: durationFilter, input_mode: "text" });
    ...
    try {
      ...
      trackAnalyticsEvent(
        "search_completed",
        { query, duration_filter: durationFilter, result_count: data.videos.length, success: true },
        elapsedAnalyticsMs(startedAt)
      );
      ...
      trackAnalyticsEvent(
        "search_failed",
        { query, duration_filter: durationFilter, success: false },
        elapsedAnalyticsMs(startedAt)
      );
    ...
    >
      ...
      >
        ...
        <Link
          ...
          aria-label="How AskTube AI works, as a guided 3D walkthrough"
        >
      ...
      >
        ...
        >
          <VideoCarousel
            ...
            onSelectVideo={(video) => {
              trackAnalyticsEvent("video_selected", {
          ...
          <ProcessingScreen
            ...
            onStart={() => {
              trackAnalyticsEvent("processing_started", { video_id: selectedVideo?.video_id });
              setIsProcessing(true);
            }}
            onComplete={() => {
              trackAnalyticsEvent("processing_completed", { video_id: selectedVideo?.video_id });
